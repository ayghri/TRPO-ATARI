...
class TRPO(object):
    ...
    def __init__(self,env,session):
        ...
        self.img_size = config.STATE_DIM[0]*config.STATE_DIM[0]
    ...
    def train(self):
        ...
        theta_old = self.current_theta()

        def fisher_vector_product(p):
            feed[self.flat_tangent] = p
            return self.session.run(self.fisher_vect_prod, feed) + config.CG_DAMP * p

        g = self.session.run(self.surr_loss_grad, feed_dict=feed)

        grad_step = utils.conjugate_gradient(fisher_vector_product, -g)
        
        sAs = .5 * grad_step.dot(fisher_vector_product(grad_step))
        
        beta_inv = np.sqrt(sAs/config.MAX_KL)
        full_grad_step = grad_step/beta_inv
        
        negdot_grad_step = -g.dot(grad_step)

        def loss(th):
            self.set_theta(th)
            return self.session.run(self.surr_loss, feed_dict=feed)
            
        theta = utils.line_search(loss, theta_old, full_grad_step, negdot_grad_step / beta_inv)
